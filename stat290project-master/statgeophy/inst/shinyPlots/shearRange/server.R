...
server <- function(input, output, session) {
      ...
      meaninput <- reactive({list(input$m1, input$m2, input$m3, input$m4, input$m5)})
      sdinput <- reactive({list(input$s1, input$s2, input$s3, input$s4, input$s5)})
      fit <- eventReactive(input$goButton, 
                               {modelFitting(
                                 generateData(datalength=lengthinput(), v_mean=meaninput(), v_sd=sdinput()),
                                 modelFit = "all")})
